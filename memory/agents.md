...
| 008  | na-009 | bpr1008-bnet | BPR bNet Product Agent — Biometric Network (BPR1008) | active | 2026-05-28 |
...
- na-004/001 cpp-face + na-003/010 multisdk — **BprFace 2.24.115 cross-platform SELF-CONTAINED natives + Linux wrappers DELIVERED (2026-07-21), resolving BOTH na-003/010 blockers above.** cpp-face built self-contained `libBprFace` for **macOS-arm64 + linux-arm64 + linux-x64** — static OpenCV 4.10.0 (bundled 3rdparty, all external optional deps OFF incl. WITH_OBSENSOR/WITH_ADE); inference is OpenCV-DNN, NO separate ONNX Runtime; no more separate libBprFaceDeps **on POSIX** — Windows still uses the split `libBprFaceDeps.dll` (MINGW whole-archive), which `libBprFace.dll` IMPORTS. macOS = PROJECTTYPE `SO_Linux`, so a static `OpenCV_DIR` self-contains with **no CMakeLists change**. Recipe committed (`src/BprIDEngine/BprFace/scripts/`, bpr.cpp `855e8af`); recorded in cpp-face knowledge mem-001. Natives + `bpr.model.onnx` (75 MB encrypted TLV blob) + `native-manifest.json` published to Generic registry (proj 230, `generic/BprFace/2.24.115/`). multisdk then built + LOAD-verified (linux/amd64) + **published** all 3 Linux wrappers: Maven `ai.bnprs:nativesdk-bprface:2.24.115` (200), NuGet `Bnprs.NativeSdk.BprFace 2.24.115` (201, Basic), Go `…/go/bprface/v2 v2.24.115` (git tag `go/bprface/v2.24.115`; `go get` resolves — **Go blocker resolved**). Two multisdk lessons: (1) loaders MUST **co-extract native + model to one temp dir** (native finds the model via its own dir); (2) **Go needs a `/v2` module-path suffix** for 2.x libs (nucleus scheme `…/go/<lib>` is wrong for v≥2). Detail: multisdk memory `bprface-2.24.115-linux-wrappers.md`. macOS/Windows wrappers not yet built (natives exist).
...
- **na-005/009 bruid-cperso + na-100/003 rnd-cperso — cPerso engine built & PROVEN LIVE (2026-07-15):** Built the full C++ M/Chip Advance personalization engine in `bpr.cpp/src/BprCardEmv/persoengine` (92 tests) and ran it end-to-end on the physical UAT Gemalto card via `perso-live --commit`. **Works live:** VISA2 SCP02 auth (diversification verified vs `gp.jar`), perso-entry (DELETE + INSTALL[make selectable] → applet instance created, the old 6999 gone), applet SCP02, and **31/41 STORE DATA DGIs accepted incl. the RSA certificate chain**. Engine has: byte-exact plaintext DGI stream (config/FCI/records from the Operas trace), P5 sequencer + assembler (41-DGI stream, P2-ordered), RSA/ODA cert builder (issuer+ICC, EMV Book 2 §6.3/6.4) with UAT test-CA mode, real symmetric key DGIs (UDKs from UAT IMKs) + ICC-key CRT, DEK-wrap, and the live PC/SC driver (over **BprPcSc** — fixed a macOS `SCardTransmit` PCI bug, merged to bpr.cpp main). **Two remaining gaps, both vendor FORMAT/SPEC** (routed to the perso bureau MENTA/Thales alongside the issuer RSA key — see rnd-cperso `07-axon-terminals/deliverables/2026-07-15-issuer-rsa-key-process.md` + task-004.1): (1) the 9 encrypted key-loading DGIs (6A80 — the `stdCPSEmvGeGKOSConfForSecretLoading` secret-loading block; format decoded, DEK choice ruled out live, needs the GeneralOS spec); (2) the SFI-14 (0E01) profile record. Full technical decode: rnd-cperso `08-memory/long-term/thales/mc-advance-dgi-map.md`; commit trail + architecture: bruid-cperso `knowledge.yaml` mem-015…mem-019. Guardrails held throughout (no key values committed — real keys by label from git-ignored `keys/uat_keystore.txt`; fail-closed key-value gate; no Claude attribution; ramaiahsvn token for bpr.cpp).
- **VISA (VSDC) also explored live (2026-07-15, bruid-cperso mem-020):** The UAT card is a multi-app Gemalto GP JavaCard (same batch/ISD key) — card manager `A000000003000000`, Visa VSDC package loaded, plus a Gemalto M/Chip instance (6999) and BNPRS BIXAPP_K3 (personalized). `perso-live-visa --commit`: installed a Visa instance `A0000000031010` (module `A0000000031056`, params **C900** — the MC params `C9050111000105` were tried and rejected), applet SCP02 OK, and **5/5 inferred plaintext DGIs accepted** (9115 AIP + 0101/0201/0301/0401 records built straight from the Visa VPA profile with the STANDARD EMV template-70 layout — no trace or proprietary config needed, cleaner than MC). Remaining for a full Visa card: keys (same secret-loading gap + **no Visa IMKs** in the keystore), AFL correctness (needs the Visa perso spec), and SET STATUS. Read-only tooling added: `card-analyze` (ATR + SELECT + GET STATUS enumeration). Visa VIS/VCPS perso spec + Visa UAT IMKs folded into the bureau ask.
- **cPerso SCOPE + manuals (2026-07-15, user directive):** the card program spans **FOUR applets — MasterCard (M/Chip Advance), Visa (VSDC), PURE (Thales platform), BIX (BNPRS BIXAPP)**; **first focus = MasterCard + Visa**. The bureau delivered the perso manuals + LoA (`Resources/Perso Manual and LoA/`: MChipAdvance GFCX17.0 ref, VSDC 2.9.2 perso, **PURE GFCX17.0 perso**, Card LoA). The PURE manual RESOLVED the secret-loading gap and **confirmed the engine's key crypto is already correct** — key DGIs 8000/8001/8010/8201-8205 are 3DES-ECB-no-pad under `SKUDEK` = the standard SCP02 session DEK we compute; the live `6A80` was a missing perso-context issue (the applet needs the full D002/D003/D004/**D005 Profile-Selection**/D007/D010-D01F/D021-D02A DGI catalog loaded before keys). Version caveat: manuals are GFCX17.0, our card is GCX7.5 — reconcile the D0xx (manual) vs A0xx (GCX7.5 trace) DGI schemes. Detail: bruid-cperso mem-021; rnd-cperso `mc-advance-dgi-map.md`.
  - **na-004/001 cpp-face + na-003/009 lib-forge + na-003/010 multisdk — BprFace 2.24.116 PUBLISHED (2026-08-01), a BEHAVIOURAL BREAK: licensing is now enforced.** Through 2.24.115 every entry point (`Bpr_FaceRecog_T12_Init`, `Bpr_FaceDetect_T12_Init`, `Bpr_SmokeDetect_T12_Init`, `Bpr_FightDetect_T12_Init`) took a `qiCode` **and** a length from the caller and ignored both, validating a constant compiled into the binary against an empty product code — **any** code passed, including empty or garbage. It was an expiry date on the build, not per-customer licensing. The fix (`8dd621a`) landed 2026-07-31, ten days after 115 was published, and the version was never bumped, so the **115 artifact in the registry and the 115 source disagree in behaviour at the same number** — hence 116 rather than a re-release. **A host passing a placeholder qiCode succeeds on 115 and fails on 116.** Verified functionally in the shipped binary (garbage/empty/truncated rejected, real licence accepted), not from the diff. Published `bpr.cpp 4276770` → generic (5 platforms: macos, linux-arm64, linux-x64, windows-32, windows-64), Maven `ai.bnprs:nativesdk-bprface:2.24.116`, NuGet `Bnprs.NativeSdk.BprFace 2.24.116`, Go tag `go/bprface/v2.24.116`; wrappers stay **Linux-only** as in 115. Three traps recorded so they are not rediscovered: (1) **the BprFace version lives in THREE files** — `bpr_versions.h`, `bpr_versions.cmake` and the root `Makefile` — and bumping only the header builds a library still *named* 2.24.115 (BprIDEngine is exempt: bengine's own CMakeLists reads the header); (2) the **115 `native-manifest.json` listed 3 binaries though the package shipped 5**, omitting both Windows platforms *and* `libBprFaceDeps.dll`, so a consumer trusting it would deploy a DLL that cannot load — 116's manifest lists all 9 files, and note raw `cmake -S/-B` does not pass `LIB_OUTPUT_DIR` so that deps DLL lands at the *build root*, not `bin/`; (3) Maven artifacts first went to `ai/bnprs/nativesdk/nativesdk-bprface/` — accepted with **200** but registered as a separate package no consumer could resolve, because the POM's `groupId` (`ai.bnprs`), not the Java package, dictates the path; corrected, wrong-path package deleted, resolution then proven by consuming it. **Correction to the 115 entry above: the GitLab Go PROXY endpoint (`…/projects/230/packages/go`) is NOT enabled on this server** — it 404s even authenticated (401 unauthenticated), including for `bprface` which was recorded as resolving. Consumption works via `GOPROXY=direct` + `GOPRIVATE='gitlab.bnprs.ai/*'` + `GOSUMDB=off` with git credentials; project 230's default branch is **`master`**, not `main`. Detail: cpp-face `knowledge.yaml` mem-012/013, lib-forge `knowledge.yaml` mem-001…004 (was an empty template), multisdk mem-006/007.
